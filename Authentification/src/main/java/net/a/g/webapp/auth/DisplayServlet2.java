...
/**
 * This Servlet demonstrates both how to obtain the Subject and then how to
 * retrieve the roles from this Subject.
 * 
 * @author Arjan Tijms
 * 
 */
		...
		try {
			...
			if (subject != null) {
				...
				for (Principal prin : subject.getPrincipals()) {
					response.getWriter().print(
							"Princpal : " + prin.toString() + "\n");
					response.getWriter().print(
							"Class : " + prin.getClass().getCanonicalName()
									+ "\n");
				}
				for (Object obj : subject.getPrivateCredentials()) {
					response.getWriter().print(
							"PrivateCredentials : " + obj.toString() + "\n");
				}
				for (Object obj : subject.getPublicCredentials()) {
					response.getWriter().print(
							"PublicCredentials : " + obj.toString() + "\n");
				}
				...
				permissionCollection.implies(new WebRoleRefPermission("",
						"nothing"));

				// Filter just the roles from all the permissions, which may
				// include things like
				// java.net.SocketPermission, java.io.FilePermission, and obtain
				// the actual role names.
		...
		return getPolicy()
				.getPermissions(
						new ProtectionDomain(new CodeSource(null,
								(Certificate[]) null), null, null, subject
								.getPrincipals().toArray(
										new Principal[subject.getPrincipals()
												.size()])));
	}

	private Set<String> filterRoles(HttpServletRequest request,
		...
		for (Permission permission : list(permissionCollection.elements())) {
			if (permission instanceof WebRoleRefPermission) {
				...
				// Note that the WebRoleRefPermission is given for every Servlet
				// in the application, even when
				// no role refs are used anywhere. This will also include
				// Servlets like the default servlet and the
				// implicit JSP servlet. So if there are 2 application roles,
				// and 3 application servlets, then
				// at least 6 WebRoleRefPermission elements will be present in
				// the collection.
